    ...
    <div class="container mt-5">
        ...
        <form action="{{ route('risks.update', $risk->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="title" class="form-control" id="title" value="{{ $risk->title }}" required>
            </div>

            <div class="mb-3">
                <label for="description" class="form-label">Description:</label>
                <textarea name="description" class="form-control" id="description" rows="4" required>{{ $risk->description }}</textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="severity" name="severity" required class="form-control">
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ $risk->severity == $i ? 'selected' : '' }}>
                            {{ $i }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="mb-3">
                ...
                <select id="occurrence" name="occurrence" required class="form-control">
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ $risk->occurrence == $i ? 'selected' : '' }}>
                            {{ $i }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="mb-3">
                ...
                <select id="detection" name="detection" required class="form-control">
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ $risk->detection == $i ? 'selected' : '' }}>
                            {{ $i }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="mb-3">
                ...
                <select id="status" name="status" required class="form-control">
                    @foreach (\App\Models\Risk::$statusDescriptions as $key => $description)
                        <option value="{{ $key }}" {{ $risk->status == $key ? 'selected' : '' }}>
                            {{ $description }}
                        </option>
                    @endforeach
                </select>
